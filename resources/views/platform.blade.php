...
<body>
<div class="bg-warning-subtle border-bottom py-2 text-center small">{{ $locale==='uz' ? 'Hayotga xavf bo‘lsa, 112 raqamiga qo‘ng‘iroq qiling.' : 'При непосредственной угрозе жизни звоните 112.' }}</div>
<nav class="navbar navbar-expand-xl bg-white border-bottom sticky-top"><div class="container">
    ...
    <div class="collapse navbar-collapse" id="nav"><ul class="navbar-nav me-auto">
        ...
        <li class="nav-item"><a class="nav-link" href="{{ route('news') }}">{{ $t['news'] }}</a></li>
    ...
    </ul><div class="d-flex align-items-center gap-2">
        ...
        @auth
            @if($isStaff)<a class="btn btn-sm btn-outline-dark" href="{{ route('admin.index') }}">{{ $t['admin'] }}</a>@endif
            <span class="small">{{ auth()->user()->name }}</span>
            <form method="post" action="{{ route('logout') }}">@csrf<button class="btn btn-sm btn-outline-secondary">{{ $t['logout'] }}</button></form>
        @else
            <a class="btn btn-sm btn-outline-secondary" href="{{ route('login') }}">{{ $t['login'] }}</a><a class="btn btn-sm btn-brand" href="{{ route('register') }}">{{ $t['register'] }}</a>
        @endauth
    </div></div>
</div></nav>
...
@if($page==='home')
    @php
        $latestNews = \Illuminate\Support\Facades\DB::table('news')->where('diaspora_id',$currentDiaspora->id)->where('is_published',true)->whereNotNull('published_at')->orderByDesc('is_pinned')->latest('published_at')->limit(3)->get();
    @endphp
    <section class="hero py-5"><div class="container py-4"><div class="row align-items-center g-4"><div class="col-lg-8">
        ...
        <a class="btn btn-light btn-lg" href="{{ route('community') }}">{{ $t['community'] }}</a> <a class="btn btn-outline-light btn-lg" href="{{ route('jobs') }}">{{ $t['jobs'] }}</a> <a class="btn btn-outline-light btn-lg" href="{{ route('news') }}">{{ $t['news'] }}</a>
    </div><div class="col-lg-4"><div class="bg-white text-dark rounded-4 p-4"><h2 class="h5">{{ $t['safety'] }}</h2><p class="small text-muted">Задержание, мошенничество, удержание документов, невыплата зарплаты, насилие.</p><a class="btn btn-danger w-100" href="{{ route('safety') }}">Открыть помощь</a></div></div></div></div></section>
    @if($latestNews->isNotEmpty())<section class="container pt-5"><div class="d-flex justify-content-between align-items-center mb-3"><h2 class="h3 mb-0">{{ $t['news'] }}</h2><a href="{{ route('news') }}">{{ $locale==='uz'?'Barcha yangiliklar':'Все новости' }}</a></div><div class="row g-4">@foreach($latestNews as $item)<div class="col-md-4"><article class="card h-100">@if($item->cover_image)<img class="news-cover rounded-top" src="{{ $item->cover_image }}" alt="">@endif<div class="card-body"><div class="small text-muted mb-2">{{ \Carbon\Carbon::parse($item->published_at)->format('d.m.Y') }}</div><h3 class="h5">{{ $tr($item->title) }}</h3><p class="text-muted">{{ Str::limit($tr($item->excerpt),160) }}</p><a class="stretched-link" href="{{ route('news.show',$item->slug) }}">{{ $locale==='uz'?'Batafsil':'Читать' }}</a></div></article></div>@endforeach</div></section>@endif
    <div class="container py-5"><div class="row g-4"><div class="col-lg-4"><h2 class="h4">{{ $t['jobs'] }}</h2>@forelse($jobs as $job)<div class="card mb-2"><div class="card-body"><strong>{{ $job->title }}</strong><div class="small text-muted">{{ $job->city }}</div></div></div>@empty<p class="text-muted">Пока нет вакансий.</p>@endforelse</div><div class="col-lg-4"><h2 class="h4">{{ $t['community'] }}</h2>@forelse($posts as $post)<div class="card mb-2"><div class="card-body"><strong>{{ $post->user_name }}</strong><p class="mb-0">{{ Str::limit($post->body,150) }}</p></div></div>@empty<p class="text-muted">Пока нет публикаций.</p>@endforelse</div><div class="col-lg-4"><h2 class="h4">{{ $t['safety'] }}</h2>@foreach($safetyArticles as $article)<div class="card mb-2"><div class="card-body"><strong>{{ $tr($article->title) }}</strong><p class="small text-muted mb-0">{{ $tr($article->summary) }}</p></div></div>@endforeach</div></div></div>

@elseif($page==='login')
    <div class="container py-5" style="max-width:560px"><div class="card"><div class="card-body p-4"><h1 class="h3">{{ $t['login'] }}</h1><form method="post" action="{{ route('login') }}">@csrf<div class="mb-3"><label class="form-label">Email / телефон</label><input class="form-control" name="login" value="{{ old('login') }}" required></div><div class="mb-3"><label class="form-label">Пароль / Parol</label><input class="form-control" type="password" name="password" required></div><button class="btn btn-brand w-100">{{ $t['login'] }}</button></form></div></div></div>

@elseif($page==='register')
    <div class="container py-5" style="max-width:780px"><div class="card"><div class="card-body p-4"><h1 class="h3">{{ $t['register'] }}</h1><p class="text-muted">Только 18+. Не публикуйте паспорт, точный адрес и банковские данные.</p><form method="post" action="{{ route('register') }}">@csrf<div class="row g-3"><div class="col-md-6"><label class="form-label">Имя / Ism</label><input class="form-control" name="name" required></div><div class="col-md-6"><label class="form-label">Телефон</label><input class="form-control" name="phone" required></div><div class="col-md-6"><label class="form-label">Email</label><input class="form-control" type="email" name="email"></div><div class="col-md-6"><label class="form-label">Дата рождения</label><input class="form-control" type="date" name="birth_date" max="{{ now()->subYears(18)->toDateString() }}" required></div><div class="col-md-6"><label class="form-label">Пол</label><select class="form-select" name="gender"><option value="male">Мужчина</option><option value="female">Женщина</option><option value="hidden">Не показывать</option></select></div><div class="col-md-6"><label class="form-label">Город</label><input class="form-control" name="city"></div><div class="col-12"><label class="form-label">Цель</label><select class="form-select" name="relationship_goal"><option value="communication">Общение</option><option value="friendship">Дружба</option><option value="family">Семья</option><option value="work">Работа</option><option value="networking">Деловые контакты</option></select></div><div class="col-md-6"><input class="form-control" type="password" name="password" placeholder="Пароль" required></div><div class="col-md-6"><input class="form-control" type="password" name="password_confirmation" placeholder="Повтор пароля" required></div><div class="col-12"><label><input type="checkbox" name="terms" value="1" required> Мне исполнилось 18 лет, принимаю правила.</label></div></div><button class="btn btn-brand mt-4">{{ $t['register'] }}</button></form></div></div></div>

@elseif($page==='community')
    <div class="container py-5"><h1 class="h2">{{ $t['community'] }} <span class="badge text-bg-secondary">18+</span></h1><form class="card card-body my-4"><div class="row g-2"><div class="col-md-4"><input class="form-control" name="city" value="{{ request('city') }}" placeholder="Город"></div><div class="col-md-3"><select class="form-select" name="gender"><option value="">Любой пол</option><option value="male">Мужчины</option><option value="female">Женщины</option></select></div><div class="col-md-3"><select class="form-select" name="goal"><option value="">Любая цель</option><option value="communication">Общение</option><option value="friendship">Дружба</option><option value="family">Семья</option><option value="work">Работа</option></select></div><div class="col-md-2"><button class="btn btn-brand w-100">{{ $t['search'] }}</button></div></div></form>
    @auth<div class="card card-body mb-4"><form method="post" action="{{ route('posts.store') }}">@csrf<div class="row g-2"><div class="col-md-3"><select class="form-select" name="type"><option value="general">Общение</option><option value="meeting">Знакомство</option><option value="work">Ищу работу</option><option value="housing">Жильё</option><option value="help">Нужна помощь</option></select></div><div class="col-md-7"><textarea class="form-control" name="body" rows="2" maxlength="5000" required></textarea></div><div class="col-md-2"><button class="btn btn-brand w-100 h-100">Опубликовать</button></div></div></form></div>@endauth
    <div class="row g-4"><div class="col-lg-8"><h2 class="h4">Лента</h2>@forelse($posts as $post)<div class="card mb-3"><div class="card-body"><strong>{{ $post->user_name }}</strong><span class="badge text-bg-light float-end">{{ $post->type }}</span><p class="mt-2 mb-0">{!! nl2br(e($post->body)) !!}</p></div></div>@empty<p>Нет публикаций.</p>@endforelse{{ $posts->links() }}</div><div class="col-lg-4"><h2 class="h4">Люди</h2><div class="row g-2">@foreach($people as $person)<div class="col-6"><div class="card h-100"><div class="card-body"><strong>{{ $person->name }}</strong><div class="small text-muted">{{ $person->city ?: 'Город не указан' }}</div><div class="small">{{ $person->profession }}</div>@auth<form method="post" action="{{ route('messages.start',$person->id) }}" class="mt-2">@csrf<button class="btn btn-sm btn-outline-primary w-100">Написать</button></form>@endauth</div></div></div>@endforeach</div>{{ $people->links() }}</div></div></div>

@elseif($page==='messages')
    <div class="container py-5" style="max-width:900px"><h1 class="h2">{{ $t['messages'] }}</h1>@forelse($conversations as $item)<a class="card my-3 text-decoration-none text-dark" href="{{ route('conversation',$item->id) }}"><div class="card-body"><strong>{{ $item->other_name }}</strong><div class="small text-muted">{{ \Carbon\Carbon::parse($item->updated_at)->diffForHumans() }}</div></div></a>@empty<div class="alert alert-light">Диалогов пока нет.</div>@endforelse{{ $conversations->links() }}</div>

@elseif($page==='conversation')
    <div class="container py-5" style="max-width:900px"><h1 class="h3">{{ $other?->name ?? 'Диалог' }}</h1><div class="card my-3"><div class="card-body" style="min-height:350px">@foreach($chatMessages as $message)<div class="mb-3 {{ $message->sender_user_id===auth()->id()?'text-end':'' }}"><div class="d-inline-block rounded-3 px-3 py-2 {{ $message->sender_user_id===auth()->id()?'own':'bg-light' }}" style="max-width:80%">{!! nl2br(e($message->body)) !!}</div></div>@endforeach</div></div><form method="post" action="{{ route('messages.send',$conversation) }}" class="card card-body">@csrf<div class="input-group"><textarea class="form-control" name="body" rows="2" maxlength="5000" required></textarea><button class="btn btn-brand">{{ $t['send'] }}</button></div></form></div>
